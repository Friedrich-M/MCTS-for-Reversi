...
class RoxannePlayer(object):
    ...
    def roxanne_select(self, node):
        """
        采用Roxanne 策略选择落子策略
        :return: 落子策略
        """

        action_list = list(node.state.get_legal_actions(self.color))
    ...
    def get_move(self, node):
        ...
        :return: 落子
        """
        action = self.roxanne_select(node)
        return action
    ...
    def stimulate_policy(self, node):
        ...
        while not self.terminal(board):
            ...
            if not len(action_list) == 0:
                roxanne1 = RoxannePlayer(color)
                action = roxanne1.get_move(node)
                # action = random.choice(action_list)
                ...
                if color == 'X':
                    ...
                    color = 'X'
                action_list = list(node.state.get_legal_actions(color))
                roxanne2 = RoxannePlayer(color)
                action = roxanne2.get_move(node)
                # action = random.choice(action_list)
                board._move(action, color)
                if color == 'X':
                    color = 'O'
